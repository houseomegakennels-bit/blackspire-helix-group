  ...
  return (
    <form onSubmit={submit} className="space-y-5">
      <div>
        ...
        <select value={sourceKey} onChange={(e) => setSourceKey(e.target.value)} className="mt-1 w-full rounded-[12px] border border-[var(--line)] bg-black/40 px-3 py-2.5 text-sm text-white outline-none">
          {sources.map((s) => (
      ...
      <div className="grid gap-4 sm:grid-cols-2">
        <div>
          ...
          <input value={county} onChange={(e) => setCounty(e.target.value)} placeholder="e.g. Guilford" className="mt-1 w-full rounded-[12px] border border-[var(--line)] bg-black/40 px-3 py-2.5 text-sm text-white outline-none placeholder:text-[var(--copy-muted)]" />
        </div>
        <div>
          <label className="block text-xs uppercase tracking-wider text-[var(--copy-muted)]">City (optional)</label>
          <input value={city} onChange={(e) => setCity(e.target.value)} placeholder="e.g. Greensboro" className="mt-1 w-full rounded-[12px] border border-[var(--line)] bg-black/40 px-3 py-2.5 text-sm text-white outline-none placeholder:text-[var(--copy-muted)]" />
        </div>
      </div>

      <div>
        <label className="block text-xs uppercase tracking-wider text-[var(--copy-muted)]">Max leads</label>
        <input type="number" min={1} max={100} value={limit} onChange={(e) => setLimit(e.target.value)} className="mt-1 w-32 rounded-[12px] border border-[var(--line)] bg-black/40 px-3 py-2.5 text-sm text-white outline-none" />
      </div>

      <div className="flex flex-wrap items-center gap-3">
        <button type="submit" disabled={busy} className="rounded-full border border-[#34d399] bg-[rgba(52,211,153,0.16)] px-6 py-3 text-sm font-semibold uppercase tracking-[0.18em] text-[#bbf7d0] transition hover:bg-[rgba(52,211,153,0.26)] disabled:opacity-50">
      ...
      {result ? (
        <div className="rounded-[16px] border border-[#34d39955] bg-black/30 p-4">
          <div className="text-sm font-semibold text-white">
            Sweep complete — {result.imported ?? 0} lead{result.imported === 1 ? "" : "s"} imported{result.scanned != null ? ` (from ${result.scanned} scanned)` : ""}.
          </div>
