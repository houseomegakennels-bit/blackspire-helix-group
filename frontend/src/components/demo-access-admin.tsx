...
type Result = { inviteUrl: string; linkExpiresAt: string; accessDays: number };
...
export function DemoAccessAdmin() {
  ...
  async function submit(event: React.FormEvent<HTMLFormElement>) {
    ...
    const response = await fetch("/api/admin/demo-access", {
      ...
      body: JSON.stringify({ label: form.get("label"), days: form.get("days"), linkHours: form.get("linkHours") }),
    });
