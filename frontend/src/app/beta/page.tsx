...
const PRIMARY_ACTIONS: Array<{ title: string; detail: string; href: string; cta: string; accent?: boolean }> = [
  {
    title: "Run a Seller Sweep",
    detail: "Pull motivated-seller leads from live county/public-record sources, score them, and load them into Seller Engine.",
    href: "/seller-engine/new",
    cta: "Launch Seller Sweep",
    accent: true,
  },
  {
    title: "Run a Buyer Sweep",
    detail: "Discover active cash buyers in a county and build a buyer report.",
    href: "/searches/new",
    cta: "Launch Buyer Sweep",
  },
  {
    title: "Review the Pipeline",
    detail: "Sentinel shows what needs attention, deal readiness, and the opportunity feed.",
    href: "/workspace/sentinel",
    cta: "Open Sentinel",
  },
  {
    title: "Try the Sample Pipeline",
    detail: "Walk a demo property through every stage - great for your first look.",
    href: "/beta/demo",
    cta: "Open Demo",
  },
];
...
export default async function BetaDashboardPage() {
  ...
  return (
    <main className="relative min-h-screen overflow-hidden bg-[radial-gradient(circle_at_top,rgba(45,212,191,0.1),transparent_34%),linear-gradient(180deg,#020403,#06090b_44%,#020303)]">
      <div className="relative z-10 mx-auto max-w-[1100px] space-y-6 px-4 py-8 lg:px-6 lg:py-10">
          ...
          <div className="mt-5 flex flex-wrap items-center gap-2">
            {PIPELINE.map((stage, i) => (
              <span key={stage} className="flex items-center gap-2">
                <span className="rounded-full border border-[var(--line)] bg-black/30 px-3 py-1 text-[11px] uppercase tracking-[0.14em] text-[var(--copy-soft)]">
                  {stage}
                </span>
        ...
        <div className="grid gap-4 md:grid-cols-2">
          {PRIMARY_ACTIONS.map((action) => (
            <div
              key={action.href}
              className="brand-panel flex flex-col justify-between p-5"
              style={action.accent ? { borderColor: "#2dd4bf55" } : undefined}
            >
              <div>
        ...
        <div className="grid gap-6 lg:grid-cols-[1fr_0.8fr]">
          <div className="brand-panel p-6">
            ...
            <ul className="mt-4 space-y-2">
              {CHECKLIST.map((item, i) => (
                <li key={item} className="flex items-start gap-3 text-sm leading-6 text-[var(--copy-soft)]">
                  <span className="mt-0.5 flex h-5 w-5 shrink-0 items-center justify-center rounded-full border border-[var(--line)] text-[10px] text-[#5eead4]">
                    {i + 1}
                  </span>
                  <span>{item}</span>
                </li>
            ...
            <div className="mt-5 flex flex-wrap gap-3">
              <Link href="/seller-engine" className="rounded-full border border-[var(--line)] px-4 py-2 text-xs uppercase tracking-[0.16em] text-[var(--copy-soft)] hover:text-white">
                Seller Engine
              </Link>
              <Link href="/buyers" className="rounded-full border border-[var(--line)] px-4 py-2 text-xs uppercase tracking-[0.16em] text-[var(--copy-soft)] hover:text-white">
                Buyer Reports
              </Link>
              <Link href="/workspace/property" className="rounded-full border border-[var(--line)] px-4 py-2 text-xs uppercase tracking-[0.16em] text-[var(--copy-soft)] hover:text-white">
                Properties
              </Link>
            </div>
          </div>
          ...
          <div className="brand-panel p-6">
            ...
            <p className="mt-3 text-sm leading-6 text-[var(--copy-soft)]">
              ...
              Any real-estate or legal documents still require human review - do not rely on generated drafts as advice.
            </p>
            <p className="mt-3 text-sm leading-6 text-[var(--copy-soft)]">
