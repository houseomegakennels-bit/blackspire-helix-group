...
const STAGES = [
  {
    ...
    facts: [["Address", "1846 Cascade St, Fayetteville, NC"], ["Asking", "$108,999"], ["Beds / Baths", "3 / 1.5"], ["Condition", "Needs roof; HVAC newer"]],
  },
  {
    engine: "Seller Engine",
    title: "Seller scored & imported",
    body: "Motivation scored from distress + equity signals.",
    facts: [["Seller", "Eric Fair"], ["Motivation", "78 / 100"], ["Equity", "~$71k"], ["Status", "Contact Ready"]],
  },
  {
    engine: "Nexus",
    title: "Contact resolved",
    body: "Skip trace returned a verified phone with confidence scoring.",
    facts: [["Phone", "(910) 555‑0142"], ["Confidence", "82 / 100"], ["DNC", "Clear"], ["Provider", "Tracerfy"]],
  },
  {
    engine: "Deal Engine",
    title: "Deal underwritten",
    body: "ARV, MAO, and assignment spread computed; readiness tracked.",
    facts: [["ARV", "$205,000"], ["MAO", "$118,500"], ["Assignment fee", "$15,000"], ["Readiness", "On Track (74)"]],
  },
  {
    engine: "Buyer Engine",
    title: "Real buyers matched",
    body: "Matched against the live buyer universe for the county.",
    facts: [["Buyers in county", "1,206"], ["Top match", "UP EIP LLC (cash)"], ["Demand", "High"], ["Action", "Send + request POF"]],
  },
];
...
export default async function BetaDemoPage() {
  ...
  return (
    <main className="relative min-h-screen overflow-hidden bg-[radial-gradient(circle_at_top,rgba(45,212,191,0.1),transparent_34%),linear-gradient(180deg,#020403,#06090b_44%,#020303)]">
      <div className="relative z-10 mx-auto max-w-[900px] px-4 py-8 lg:px-6 lg:py-10 space-y-6">
        <div className="flex items-center gap-3 text-xs">
          <Link href="/beta" className="text-[#5eead4] hover:underline">← Beta home</Link>
        </div>

        <div className="brand-panel p-6 lg:p-7">
          <span className="rounded-full border border-amber-400/30 bg-amber-500/10 px-3 py-1 text-[10px] uppercase tracking-[0.2em] text-amber-200">Sample / demo data</span>
          ...
          <p className="mt-2 text-sm leading-7 text-[var(--copy-soft)]">
            ...
            you're ready, run a real <Link href="/seller-engine/new" className="text-[#5eead4] hover:underline">Seller Sweep</Link>.
          </p>
        </div>
          ...
          {STAGES.map((stage, i) => (
            <div key={stage.engine} className="brand-panel p-5">
              <div className="flex items-center gap-3">
                <span className="flex h-7 w-7 shrink-0 items-center justify-center rounded-full border border-[#2dd4bf] text-xs font-black text-[#5eead4]">{i + 1}</span>
                <div>
        ...
        <div className="brand-panel p-6 text-center">
          ...
          <Link href="/seller-engine/new" className="mt-3 inline-flex rounded-full bg-[#34d399] px-6 py-3 text-xs uppercase tracking-[0.18em] text-[#020403]">Run a real Seller Sweep →</Link>
        </div>
      </div>
      <BetaFeedback />
    </main>
