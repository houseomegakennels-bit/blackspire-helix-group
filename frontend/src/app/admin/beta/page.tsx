...
export default async function AdminBetaPage() {
  ...
  return (
    <main className="relative min-h-screen overflow-hidden bg-[radial-gradient(circle_at_top,rgba(45,212,191,0.1),transparent_34%),linear-gradient(180deg,#020403,#06090b_44%,#020303)]">
      <div className="relative z-10 mx-auto max-w-[1200px] px-4 py-8 lg:px-6 lg:py-10 space-y-6">
                ...
                snap.feedback.map((f) => (
                  <div key={f.id} className="rounded-[14px] border border-[var(--line)] p-3">
                    <div className="flex items-center gap-2">
                      <span className="rounded-full px-2 py-0.5 text-[9px] uppercase tracking-wider" style={{ color: CATEGORY_COLOR[f.category] ?? "#94a3b8", background: `${CATEGORY_COLOR[f.category] ?? "#94a3b8"}1f` }}>
                        {f.category.replaceAll("_", " ")}
                      </span>
                      <span className="truncate text-xs text-[var(--copy-muted)]">{f.email ?? "anon"} · {fmt(f.createdAt)}</span>
                    </div>
