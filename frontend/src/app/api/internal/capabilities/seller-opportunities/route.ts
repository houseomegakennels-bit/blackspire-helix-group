...
export async function POST(request: NextRequest) {
  ...
  if (!authorizeInternalCapability(request, body.workspaceId)) return NextResponse.json({ ok: false, error: "not found" }, { status: 404 });
  if (Object.keys(body).some((key) => !["workspaceId", "limit"].includes(key))) return NextResponse.json({ ok: false, error: "invalid request" }, { status: 400 });
  ...
  const opportunities = leads.slice(0, limit).map((lead) => ({
