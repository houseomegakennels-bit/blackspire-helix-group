...
export default async function WorkflowsPage() {
  ...
  const blockedCount = counties.filter((county) => county.status === "inactive").length;
  ...
  const blockedCounties = counties.filter((county) => county.status === "inactive");
      ...
      <div className="grid gap-6 xl:grid-cols-[1.2fr_0.8fr]">
        ...
        >
          <div className="space-y-3">
            {workflows.map((workflow) => (
              <div key={workflow.workflowId} className="brand-card p-4">
                <div className="flex flex-col gap-3 lg:flex-row lg:items-start lg:justify-between">
                  <div className="space-y-2">
                    ...
                    <p className="brand-copy-soft text-sm leading-6">{workflow.summary}</p>
                  </div>
                  <dl className="brand-copy-soft min-w-72 space-y-1 text-sm">
                    <div className="flex justify-between gap-4">
                      <dt>Workflow ID</dt>
                      <dd className="font-mono text-xs text-white">{workflow.workflowId}</dd>
                    </div>
                    {workflow.webhookPath ? (
                      <div className="flex justify-between gap-4">
                        <dt>Webhook</dt>
                        <dd className="font-mono text-xs text-white">{workflow.webhookPath}</dd>
                      </div>
                    ) : null}
                    {workflow.recentRuns ? (
                      <div className="flex justify-between gap-4">
                        <dt>Recent runs</dt>
                        <dd className="max-w-56 text-right text-xs text-white">{workflow.recentRuns}</dd>
                      </div>
                    ) : null}
                  </dl>
                </div>
        ...
        >
          <div className="space-y-3">
            ...
            <div className="brand-card brand-copy-soft p-4 text-sm leading-6">
              <div className="text-xs uppercase tracking-[0.24em] text-[var(--copy-muted)]">Approved for 90-day sweeps</div>
            ...
            <div className="brand-card brand-copy-soft p-4 text-sm leading-6">
              <div className="text-xs uppercase tracking-[0.24em] text-[var(--copy-muted)]">Visible but not approved for 90-day sweeps</div>
              <div className="mt-3 flex flex-wrap gap-2">
            ...
            <div className="brand-card brand-copy-soft p-4 text-sm leading-6">
              <div className="text-xs uppercase tracking-[0.24em] text-[var(--copy-muted)]">Blocked now</div>
              <div className="mt-3 flex flex-wrap gap-2">
                {blockedCounties.map((county) => (
                  <StatusPill key={county.county} tone="bad" label={county.county} />
                ))}
              </div>
              <p className="mt-3">
