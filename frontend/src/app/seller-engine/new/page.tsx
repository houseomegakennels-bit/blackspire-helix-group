...
export default async function NewSellerSweepPage() {
  ...
  return (
    <main className="relative min-h-screen overflow-hidden bg-[radial-gradient(circle_at_top,rgba(52,211,153,0.1),transparent_34%),linear-gradient(180deg,#020403,#06090b_44%,#020303)]">
      <div className="relative z-10 mx-auto max-w-[760px] space-y-6 px-4 py-8 lg:px-6 lg:py-10">
        <div className="flex items-center gap-3 text-xs">
          <Link href="/beta" className="text-[#5eead4] hover:underline">
            &larr; Beta home
          </Link>
          <Link href="/seller-engine" className="text-[var(--copy-muted)] hover:text-white">
            Seller Engine
          </Link>
        </div>
        ...
        <div className="brand-panel p-6 lg:p-7">
          ...
          <p className="mt-2 text-sm leading-7 text-[var(--copy-soft)]">
            ...
            them straight into Seller Engine. Looking to import a spreadsheet instead? Use{" "}
            <Link href="/seller-engine" className="text-[#5eead4] hover:underline">
              Import Seller CSV
            </Link>{" "}
            in Seller Engine.
          </p>
