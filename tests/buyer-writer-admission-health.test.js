 ...
 class Pool extends EventEmitter{
  ...
  async connect(){
   ...
   const client={calls:[],async query(config){
    this.calls.push(config);return respond({pool,client,config});
...
test('ready proves current database and template1 and returns only sanitized status',async()=>{
 const {Pool,instances}=harness(safeResponse);
 ...
 try{
  ...
  assert.equal(current[0].calls[0].text,ADMISSION_IDENTITY_SQL);
  assert.deepEqual(current[0].calls[0].values,[BUYER_WRITER_ADMISSION_LOGIN,16384]);
...
test('unsafe current or template proof is sanitized and destroys only the unsafe session',async()=>{
 for(const unsafe of ['current','template']){
  ...
  const {Pool,instances}=harness(({config})=>{
   ...
   if(config.text===ADMISSION_IDENTITY_SQL)return {rows:[{safe:unsafe!=='current'}]};
  ...
  try{
   ...
   const current=instances[0].clients[0];
   assert.equal(current.destroyed,unsafe==='current');
   if(unsafe==='template'){
    assert.equal(instances[1].clients[1].destroyed,true);
   }else{
    assert.equal(instances[1].clients.length,1);
   }
  }finally{await database.close();}
 }
});
...
test('aborted probe fails closed quickly and destroys its checked-out session',async()=>{
 ...
 const {Pool,instances}=harness(({config})=>{
  ...
  if(config.text===ADMISSION_IDENTITY_SQL)return new Promise(resolve=>{hold=resolve;});
  throw new Error('unexpected SQL');
 });
 const database=await createBuyerWriterAdmissionPostgres({connection:connection(),expectedCreatorOid:16384,Pool});
 try{
...
test('probe query deadline is bounded, sanitized and destroys the session',async()=>{
 const {Pool,instances}=harness(({config})=>{
  if(config.text===ADMISSION_TEMPLATE1_IDENTITY_SQL)return {rows:[{safe:true}]};
  if(config.text===ADMISSION_IDENTITY_SQL)return new Promise(()=>{});
  throw new Error('unexpected SQL');
 });
 const database=await createBuyerWriterAdmissionPostgres({connection:connection(),expectedCreatorOid:16384,Pool});
 try{
