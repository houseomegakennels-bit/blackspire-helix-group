...
test('production startup refuses an unsafe operational integer before serving traffic', async () => {
  const result = await runApi({
    NODE_ENV: 'production',
    BLACKSPIRE_DB_PATH: path.join(root, 'unsafe-integer', 'command.sqlite'),
    TELEGRAM_TMP_DIR: path.join(root, 'unsafe-integer-attachments'),
    PORT: '8901',
  ...
  assert.equal(result.code, 1);
  assert.match(result.stderr, /CLEANUP_INTERVAL_MS/);
});
