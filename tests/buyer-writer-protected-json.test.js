...
function fixture({file={},directory={},contents='{"version":1}',changed=false,failure,aclResult,reader=readRootOwnedJson,maxBytes=256}={}){
  ...
  const io={
    ...
    readSync:(_fd,buffer,offset,length)=>{rawBuffer=buffer;if(failure==='read')throw new Error('PRIVATE_SECRET');const n=Math.min(length,bytes.length-position);bytes.copy(buffer,offset,position,position+n);position+=n;return n;},
    closeSync:()=>{closed++;if(failure==='close')throw new Error('PRIVATE_SECRET');},
  };
  return {read:filename=>reader(filename??'/etc/blackspire/writer.json',{io,groupId:42,maxBytes,aclTool:(command,args,options)=>{assert.equal(command,'/usr/bin/getfacl');assert.equal(args.at(-1),'/proc/self/fd/3');assert.equal(options.stdio[3],7);assert.equal(options.timeout,250);assert.equal(options.maxBuffer,4096);return aclResult??{status:0,stdout:'',stderr:''};}}),closed:()=>closed,bytesRead:()=>position,rawBuffer:()=>rawBuffer};
}
...
test('malformed content and I/O errors expose no credential-bearing cause',()=>{
  ...
  for(const failure of ['open','read','close']){
    const f=fixture({failure});assert.throws(()=>f.read(),error=>error.message==='Buyer writer protected configuration unavailable'&&!error.cause);
    if(f.rawBuffer())assert.ok(f.rawBuffer().every(byte=>byte===0),'raw protected JSON buffer is wiped after I/O failure');
  }
...
test('large offline metadata has a distinct fixed bound without enlarging the credential ceiling',()=>{
 const contents=JSON.stringify({catalog:'x'.repeat(450000)});
 const good=fixture({contents,reader:readRootOwnedMetadataSnapshot});
 assert.equal(good.read().value.catalog.length,450000);assert.equal(good.closed(),1);
 const credential=fixture({contents,maxBytes:2*1024*1024});
 assert.throws(()=>credential.read(),/unavailable/);assert.equal(credential.bytesRead(),0);
 const excessive=fixture({contents:JSON.stringify({catalog:'x'.repeat(2*1024*1024)}),reader:readRootOwnedMetadataSnapshot});
 assert.throws(()=>excessive.read(),/unavailable/);assert.equal(excessive.bytesRead(),0);assert.equal(excessive.closed(),1);
 for(const extra of [{changed:true},{file:{nlink:2}},{directory:{mode:0o40777}},{aclResult:{status:0,stdout:'user:993:r--',stderr:''}}]){
  assert.throws(()=>fixture({contents,reader:readRootOwnedMetadataSnapshot,...extra}).read(),/unavailable/);
 }
});
