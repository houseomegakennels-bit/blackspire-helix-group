...
import {lookupBuyerWriterIdentity,resolveBuyerWriterIdentity} from '../packages/buyer-writer/runtime-identity.js';
...
test('fixed identity lookup does not impersonate the API process',async()=>{
  const {lookup}=fixture();
  assert.deepEqual(await lookupBuyerWriterIdentity({lookup}),
    {uid:994,credentialGroupId:984,workerUid:993});
});
