...
import {PREDECESSOR_MAIN,REVIEWED_FEATURE_MAIN,SUCCESSOR_MAIN,bindSuccessorMain,observeSuccessorMain} from '../packages/zola-release/owned-successor-main.js';
...
test('fixed trusted remote and all ancestry edges are required',()=>{
 ...
 assert.deepEqual(calls.slice(1).map(a=>a.slice(-4)),[['merge-base','--is-ancestor',PREDECESSOR_MAIN,REVIEWED_FEATURE_MAIN],['merge-base','--is-ancestor',REVIEWED_FEATURE_MAIN,SUCCESSOR_MAIN],['merge-base','--is-ancestor',SUCCESSOR_MAIN,sha]]);
 for(const main of [PREDECESSOR_MAIN,'a'.repeat(40)])assert.throws(()=>observeSuccessorMain(sha,{run:()=>`${main}\trefs/heads/main`}));
 for(const edge of [PREDECESSOR_MAIN,REVIEWED_FEATURE_MAIN,SUCCESSOR_MAIN])assert.throws(()=>observeSuccessorMain(sha,{run:(bin,args)=>{if(args.includes('ls-remote'))return `${SUCCESSOR_MAIN}\trefs/heads/main`;if(args.at(-2)===edge)throw Error('not ancestor');return '';}}));
});
