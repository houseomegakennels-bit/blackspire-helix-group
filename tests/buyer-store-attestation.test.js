...
test('installed artifact and observed process generations gate repository requests',async()=>{
 ...
 let generation=manifest.apiGeneration+'\n'+manifest.workerGeneration,artifactCalls=0;
 const deps={read:()=>({value:manifest,identity:{uid:0,gid:process.getgid(),mode:0o640},digest:'e'.repeat(64)}),io:{realpathSync:()=>artifactRoot},run:()=>generation,inspect:async()=>{artifactCalls++;return {artifactDigest:manifest.artifactDigest};},moduleRoot:artifactRoot};
