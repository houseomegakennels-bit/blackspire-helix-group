...
for(const mode of ['--release-cloud-proof-attempt5','--release-cloud-proof-attempt6','--release-cloud-proof-attempt7','--release-cloud-proof-attempt8'])test(`actual successor launcher dispatches ${mode} only after source gates`,()=>{
...
test('actual successor launcher rejects unknown mode before git or dynamic dispatch',()=>{
 const result=execute('--release-cloud-proof-attempt9');assert.equal(result.exitCode,1);assert.deepEqual(result.events,[]);
