...
test('source transformation refuses drift and changes only original version predicate',()=>{
 const output=transformOwnedSixReadSource(source),start=output.indexOf("import fs from 'node:fs';");
 assert.equal(output.slice(start),source.replace('config.version!==4','!ownedSixReadConfigMatches(config,context)'));
...
test('source guard rejects frozen SHA/path/dirty and non-descendant wrapper identities',()=>{
 const v={wrapperRoot:'/mnt/blackspire-builds/development-cache/0/workspaces/zola-owned-premerge-six-permit-20260923',frozenRoot:fixed.frozenRoot,frozenSha:fixed.frozenSha,wrapperClean:true,frozenClean:true,ancestor:true,wrapperSha:'a'.repeat(40)};
