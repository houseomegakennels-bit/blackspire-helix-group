...
import {ownedN8nAttempt4SupervisorArguments,validateOwnedN8nAttempt4Supervisor,awaitOwnedN8nAttempt4SupervisorAcknowledgment} from '../packages/zola-release/owned-n8n-cloud-attempt4-supervisor.js';
test('READY and completion bind exact supervisor intent, unit, PID start and invocation',()=>{
 const f=interruptedFixture(),plan={...f.records.plan,attempt:4,predecessorInterruptionDigest:'f'.repeat(64)},operatorRoot='/mnt/blackspire-builds/development-cache/0/workspaces/zola-owned-cloud-attempt4-20260921',root='/var/lib/blackspire-operator/preparation/owned-n8n-cloud-proof/a8e05ef40e44b6695df5b30356af0e411fe36f1a-c8b00904-7017-434a-918e-8aaadbae82fd-attempt4';
 ...
 const intent={version:1,planDigest:hash(plan),unit:'zola-n8n-cloud-proof-attempt4.service',argumentsDigest:hash(ownedN8nAttempt4SupervisorArguments({operatorRoot,root}))},process={pid:1234,uid:0,startTime:'987654',command},input={plan,operatorRoot,root,intent,properties,process,phase:'launch'};
...
test('child-first launch waits for exact parent acknowledgment without service redispatch',async()=>{
 ...
 const ready=await awaitOwnedN8nAttempt4SupervisorAcknowledgment({pid:1234,observe:()=>{observations++;return {...identity};},readResult:()=>result,validateReady:()=>{assert.ok(result);return {...identity};},now:()=>clock,pause:async ms=>{clock+=ms;if(++waits===delay)result={retained:true};}});
