...
for(const owned of [false,true])test(`installed ${owned?'owned':'legacy'} manifest composes with real binding and HELD readiness`,{skip:process.getuid()!==0},async t=>{
 ...
 const databaseProfile={version:1,...OWNED_POSTGRES_TARGET,creatorOid:source.creatorOid,systemIdentifier:'2234567890123456789',caSha256:hash(ca)};
 const backend=owned?{backendProfile:'owned-postgres-v1',profileDigest:ownedPostgresProfileDigest(databaseProfile)}:{};
 if(owned){source.runtime={...source.runtime,...backend,host:'127.0.0.1',port:55432};source.issuer={...source.issuer,...backend,host:'127.0.0.1',port:55432};}
 ...
 const profileOptions={readDatabaseProfile:()=>databaseProfile,configDirectory,gatewayDirectory,unitDirectory,resolveIdentity:async()=>({uid:994,credentialGroupId:984,workerUid:993}),capture:collect,inspectFactory:()=>inspectRuntime,
  run:(_file,args)=>args[0]==='group'?'blackspire-writer:x:982:\n':String(api.supervisor.pid)+'\n'};
 const profile=await collectInstalledHeldWriterProfile(releaseSha,profileOptions);assert.equal(profile.configurationDigest.length,64);assert.equal(profile.context.apiPid,api.child.pid);assert.equal(profile.backendProfile,backend.backendProfile);assert.equal(profile.profileDigest,backend.profileDigest);
 ...
 const ready={ok:false,service:'blackspire-command-api',lifecycle:'ready',database:'compatible',productionConfig:{ok:true},checks:{...(owned?{buyerStore:true}:{}),releaseAdmission:false,lifecycle:true,database:true,productionConfig:true,worker:true,scheduler:true,deploymentIdentity:true},dependencies:{worker:workerHealth,scheduler:{ok:true}},deploymentIdentity:identity};
 const observeBinding=requireCommit=>createBuyerWriterBindingObserver({...profile.context,inspectRuntime,requireCommit});
 const base={...backend,workspace:'blackspire-command',releaseSha,apiGeneration,environment:'production',getHealth:()=>health,getReadiness:()=>ready};
...
inspectArtifact:async()=>({releaseSha,environment:'production',artifactDigest:manifest.artifactDigest}),
   ...
   checkReadiness:options=>checkBuyerWriterHeldReadiness({...options,...backend,requirePreparation:options.requirePreparation||options.requireWriterReady,preparationCredential:profile.preparationCredential,verifyHeld:()=>lease.assertIdentity()})});
  ...
  if(owned){ready.checks.buyerStore=false;assert.equal((await fetch(`http://127.0.0.1:${context.port}/api/internal/buyer-writer/v1/preparation`,{headers:{'x-buyer-issuer-key':source.issuerCredential}})).status,503);ready.checks.buyerStore=true;}
