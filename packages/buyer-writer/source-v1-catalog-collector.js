...
function management(value,target){
...
export async function collectBuyerWriterSourceV1Catalog({releaseSha,artifactDigest,
  ...
  try{
    if(!SHA.test(releaseSha??'')||!DIGEST.test(artifactDigest??'')
      ...
      ||!exact(target,['host','port','database','ca'])
      ||target.port!==5432||target.database!=='postgres'
      ||typeof connect!=='function'||typeof now!=='function')fail();
    const credential=management(managementConfiguration,target);
    ...
    return Object.freeze({version:1,kind:'buyer-writer-authenticated-catalog-evidence',
      releaseSha,environment:'production',workspace:'blackspire-command',artifactDigest,
      credentialSourceDigest,capturedAt,
      target:Object.freeze({host:target.host,port:target.port,database:target.database,serverMajor:17}),
