...
export async function inspectBuyerWriterArtifact({artifactRoot,releaseSha,environment,run=execute}){
  try{
    ...
    const result=await run('/opt/nodejs/node-v22.23.1-linux-x64/bin/node',['--max-old-space-size=128',worker,artifactRoot,releaseSha,environment],{
    ...
    if(!value||Array.isArray(value)||Object.keys(value).length!==3||value.releaseSha!==releaseSha||value.environment!==environment
