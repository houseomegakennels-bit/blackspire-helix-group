...
export async function createBuyerWriterRuntime({configurationFile,workspace,releaseSha,apiGeneration,environment,getHealth,getReadiness,
  ...
  try {
    ...
    const config=validateBuyerWriterConfiguration(readConfiguration(configurationFile,{groupId:identity.credentialGroupId,maxBytes:65536}),{workspace});
    ...
    const observeBinding=createBinding({filename:config.bindingFile,credentialGroupId:identity.credentialGroupId,workspace,releaseSha,apiGeneration,
      apiUid:identity.uid,workerUid:identity.workerUid,inspectRuntime:createBuyerWriterRuntimeInspector()});
    ...
    const handler=createBuyerWriterRequestHandler({credential:config.writerCredential,workspace,query:database.runtimeQuery,isAvailable:checkAvailability,
      issuer:{credential:config.issuerCredential,query:database.issuerQuery}});
    return Object.freeze({...handler,checkAvailability,isHealthy:()=>!closed&&database.isHealthy()===true,close:()=>{
