...
import { canReadEvaluation } from '../shared/authorization.js';
...
export function deriveVerifiedScorecards(principal, { workspaceId, cutoff, scorecardVersion = SCORECARD_VERSION } = {}) {
  ...
  return transaction(() => {
    ...
    // Authorization is checked once, against the caller-named workspace, before any evidence is
    // read. Every selected source must then already belong to that workspace, so a caller can
    // neither widen scope nor have their input override the scope stored in the evidence.
...
function validatedSource(evaluation, workspaceId, cutoff, derivationEpoch) {
  ...
  for (const event of events) if (!(event.event_type in EVENT_COUNTERS)) throw new Error('verified scorecard refuses an unknown source event type');
  return { evaluation, events, correctionHead, dimensions: dimensionsOf(evaluation) };
}
...
// and never a collapsed empty string.
function dimensionsOf(evaluation) {
  const classification = parsedJson(evaluation.classification);
  const routing = evaluation.routing_decision_id ? getRoutingDecision(evaluation.routing_decision_id) : null;
  ...
  const capabilities = classification && Array.isArray(classification.requiredCapabilities) ? [...classification.requiredCapabilities].sort(compareStrings) : null;
  const dimensions = {
...
function deriveMetrics(sources) {
  const metrics = {
    ...
    retry_total: 0, timeout_count: 0, cancellation_count: 0,
  ...
  for (const { evaluation, events } of sources) {
    ...
    const acceptanceEvents = [...eventTypes].filter((type) => ACCEPTANCE_EVENTS.has(type));
    ...
    if (evaluation.timed_out === 1) metrics.timeout_count += 1;
...
function persistGroup(group, workspaceId, cutoff, scorecardVersion) {
  ...
  // The lineage digest binds the version, the exact scope and dimensions, the cutoff, every ordered
  // evaluation id and provenance digest, each selected correction head, the ordered source-event
  // ids, and every persisted metric. Neither the snapshot id nor its wall-clock creation time is
  // hashed, so an identical re-derivation reproduces an identical digest.
  const packet = { scorecardVersion, derivationVersion: SCORECARD_DERIVATION_VERSION, scope, cutoff: { createdAt: cutoff.createdAt, id: cutoff.id }, lineage, metrics };
  if (!digestibleValue(packet)) throw new Error('verified scorecard refuses a non-canonical lineage packet');
  const lineageDigest = digest(packet);
  const contentDigest = digest({ scorecardVersion, derivationVersion: SCORECARD_DERIVATION_VERSION, confidenceBand: confidenceBand(metrics.source_evaluation_count), metrics });
  ...
  const row = {
    ...
    confidence_band: confidenceBand(metrics.source_evaluation_count),
...
// A stored snapshot is only readable when its own persisted lineage still reproduces its recorded
// content digest and its lineage rows are a complete, gap-free, correctly ordered sequence.
function storedScorecardIntact(scorecard) {
  ...
  return scorecard.content_digest === digest({ scorecardVersion: scorecard.scorecard_version, derivationVersion: scorecard.derivation_version, confidenceBand: scorecard.confidence_band, metrics });
}
