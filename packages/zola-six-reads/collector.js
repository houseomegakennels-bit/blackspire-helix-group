import {validateOwnedDivisionSnapshot,validateOwnedOwnerWitness,compareOwnedDivisionSnapshots} from './owned-database-observer.js';
...
export const collectorCompatibilityVersion=config=>config?.version===6?4:config?.version===7?5:config?.version;
export const ownedCollector=config=>[6,7].includes(config?.version);
...
export function validateCollectorConfig(value) {
  ...
  if ([2,3,4,5,6,7].includes(value?.version)) keys.push('observerDatabaseConfigPath');
  if ([4,5,6,7].includes(value?.version)) keys.push('denialReceiptPath');
  if ([5,7].includes(value?.version)) keys.push('releaseRunId');
  if(ownedCollector(value))keys.push('backendProfile','profileDigest','ownedObserverDatabaseConfigPath','acceptanceSearchJobId');
  if ([4,5,6,7].includes(value?.version) && (typeof value.denialReceiptPath !== 'string' || !value.denialReceiptPath.startsWith('/') || value.denialReceiptPath.includes('\0'))) refuse('INVALID_CONFIGURATION');
  if (!value || Array.isArray(value) || Object.keys(value).length !== keys.length || keys.some(k => !Object.hasOwn(value,k)) || ![1,2,3,4,5,6,7].includes(value.version) || ([2,3,4,5,6,7].includes(value.version) && (typeof value.observerDatabaseConfigPath !== 'string' || !value.observerDatabaseConfigPath.startsWith('/') || value.observerDatabaseConfigPath.includes('\0'))) ||
  ...
  if([5,7].includes(value.version)&&!id(value.releaseRunId))refuse('INVALID_CONFIGURATION');
  if(ownedCollector(value)&&(value.backendProfile!=='owned-postgres-v1'||!/^[a-f0-9]{64}$/.test(value.profileDigest??'')||value.ownedObserverDatabaseConfigPath!=='/etc/blackspire/owned-postgres/management.json'||value.observerDatabaseConfigPath!=='/etc/blackspire-buyer-writer-gateway/management.json'||!/^[a-f0-9]{8}-(?:[a-f0-9]{4}-){3}[a-f0-9]{12}$/.test(value.acceptanceSearchJobId??'')))refuse('INVALID_CONFIGURATION');
...
export function requireProductionCollectorConfig(config) {
  if (![5,7].includes(config?.version)) refuse('PRODUCTION_RECEIVER_AUTHORITY_REQUIRED');
  return config;
}
...
export function verifyCollectedTask({ task, attempts }, config, entry, key, generation) {
  ...
  if([5,6,7].includes(config.version)){
    ...
    if(authority.releaseSha!==config.releaseSha||authority.releaseRunId!==(config.version===6?config.runId:config.releaseRunId)||authority.apiGeneration!==generation.apiGeneration
  ...
  const decoded = decodeObservedResponse(JSON.stringify(result), new Response(null, { headers: { 'x-zola-read-observation': JSON.stringify(header),
    ...(receiverAuthorityDigest?{'x-blackspire-authority-binding':receiverAuthorityDigest}:{}) } }), route,[5,6,7].includes(config.version)?receiverAuthorityDigest:null);
  const observed = observationForResult(decoded);
  if (!observed) refuse('MISSING_OBSERVATION');
  if(ownedCollector(config)&&entry.capability.startsWith('buyer.')&&observed.version!==2)refuse('OWNED_BUYER_TRANSPORT_REQUIRED');
...
export async function collectSixReads(config, host, store) {
  ...
  if ([3,4,5,6,7].includes(config.version) && existing.some(e => e.type === 'database_query_intent' && e.binding?.phase === 'after')) refuse('CONNECTED_OBSERVER_INTERVAL_CLOSED');
  if (!existing.length) store.append({ type: 'run', binding, releaseSha: config.releaseSha });
  const generation = await host.generation();
  const acceptance=[4,5,6,7].includes(config.version)&&typeof host.acceptance==='function'?await host.acceptance(generation):null;
  if([4,5,6,7].includes(config.version)&&!acceptance)refuse('HELD_ACCEPTANCE_BINDING_MISMATCH');
  const sameGeneration = async () => { if (JSON.stringify(await host.generation()) !== JSON.stringify(generation)) refuse('GENERATION_CHANGED'); };
  ...
  let databaseBefore;
  if ([2,3,4,5,6,7].includes(config.version)) {
    ...
    if (!beforeEvents.length) {
      ...
      (ownedCollector(config)?validateOwnedDivisionSnapshot:validateDivisionSnapshot)(databaseBefore.snapshot, config, 'before');
      (ownedCollector(config)?validateOwnedOwnerWitness:validateOwnerWitness)(databaseBefore.owner, config, 'before');
      ...
      if ([3,4,5,6,7].includes(config.version) && digest(await host.observeDatabase('before', { generation, store })) !== digest(databaseBefore)) refuse('CONNECTED_OBSERVER_BASELINE_MISMATCH');
      validateDatabaseEnvelope(databaseBefore);
      (ownedCollector(config)?validateOwnedDivisionSnapshot:validateDivisionSnapshot)(databaseBefore.snapshot, config, 'before');
      (ownedCollector(config)?validateOwnedOwnerWitness:validateOwnerWitness)(databaseBefore.owner, config, 'before');
    }
  ...
  for (const [index, entry] of readCases(config.dealId).entries()) {
    await sameGeneration();
    const key = `zola-six:${[5,7].includes(config.version)?config.releaseRunId:config.runId}:${index}`;
  ...
  if ([2,3,4,5,6,7].includes(config.version)) {
    ...
    (ownedCollector(config)?validateOwnedOwnerWitness:validateOwnerWitness)(after.owner, config, 'after');
    if (after.owner.witness !== databaseBefore.owner.witness) refuse('DATABASE_OWNER_WITNESS_CHANGED');
    databaseEvidence = { ...(ownedCollector(config)?compareOwnedDivisionSnapshots:compareDivisionSnapshots)(databaseBefore.snapshot, after.snapshot, config),
  ...
  const livePass=Boolean([5,7].includes(config.version)&&acceptance&&databaseEvidence&&results.length===6&&results.every(row=>row.authorityVersion===1&&row.mutationDelta===0)
  ...
  const report = { version: 1, releaseSha: config.releaseSha, collectedAt: new Date().toISOString(),
    ...
    ...(acceptance?([4,6].includes(config.version)?{premergeAcceptance:acceptance}:{heldAcceptance:acceptance}):{}),
    ...
    ...([5,7].includes(config.version)?{receiverAuthorityPass:results.every(row=>row.authorityVersion===1)}:{}) };
