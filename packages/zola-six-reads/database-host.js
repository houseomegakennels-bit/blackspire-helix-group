...
export function createProductionDatabaseObserver(config) {
  return async phase => {
    const client = createFixedNativeObserverClient(config);
    client.on('error',()=>{});
    try {
...
// Shared fixed native transport; no ambient connection string or TLS override.
export function createFixedNativeObserverClient(config) {
    ...
    return new pg.Client({ host: credential.host, port: 5432, user: 'postgres', database: 'postgres', password: credential.password,
