...
export async function runSubscriptionCodexWorker(request, { spawnImpl = spawn, timeoutMs = 45_000 } = {}) {
  ...
  try {
    ...
    return { ok: true, status: 'completed', worker: 'codex-subscription', authenticationMode: 'chatgpt-subscription', invocationCount: 1, toolCalls: 0, result: redact(parsed.summary), contractVersion: parsed.version, diagnostic:diagnosticMetadata({ result, timedOut, stdout, stderr, parserStage, startedAt }) };
  } catch (error) {
...
export function validateWorkerResponse(response, request) {
  exactKeys(response, ['version','status','summary'], 'worker response');
  if (response.version !== CODEX_WORKER_CONTRACT_VERSION) throw new Error('worker response changed contract version');
  if (response.status !== 'operational' || response.summary !== 'Restricted subscription Codex worker path is operational.') throw new Error('invalid worker result');
  return response;
}
...
function workerPrompt(request) {
  return `You are a restricted read-only worker. Do not call tools, run commands, access files, browse, modify anything, or contact external resources. The synthetic fixture is: {"service":"restricted-codex-worker","state":"operational","sensitive":false}. Objective: ${request.objective}\nReturn only the JSON object required by the supplied schema with version 1, status operational, and the required concise summary.`;
}
function responseSchema() { return { type:'object', additionalProperties:false, required:['version','status','summary'], properties:{ version:{type:'string',enum:['1']}, status:{type:'string',enum:['operational']}, summary:{type:'string',enum:['Restricted subscription Codex worker path is operational.']} } }; }
...
function structuredEventTypes(value) { return [...new Set(String(value||'').split('\n').flatMap((line)=>{ try { const event=JSON.parse(line); const type=String(event.type||event.item?.type||event.status||'unknown'); return /^[a-z0-9._-]{1,64}$/i.test(type)?[type]:[]; } catch { return []; } }))].slice(0,16); }
