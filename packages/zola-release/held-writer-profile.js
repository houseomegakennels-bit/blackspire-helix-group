...
export async function collectInstalledHeldWriterProfile(releaseSha){
 if(process.getuid()!==0||!/^[a-f0-9]{40}$/.test(releaseSha??''))fail();
 const identity=await lookupBuyerWriterIdentity(),snapshots=[];
 const read=(file,groupId,maxBytes=65536)=>{const snapshot=readRootOwnedJsonDigestSnapshot(file,{groupId,maxBytes});snapshots.push({file,groupId,maxBytes,snapshot});return snapshot;};
 const manifest=read(installedBuyerWriterManifestPath(releaseSha),0,16384).value;
 ...
 for(const key of ['clientConfig','ingressConfig','signerConfig','gatewayConfig','serviceDropin']){
  ...
  if(['clientConfig','ingressConfig','signerConfig'].includes(key)&&!new RegExp('^/etc/blackspire/buyer-writer-'+key.replace('Config','')+'-[a-f0-9]{64}\\.json$').test(ref.path))fail();
 }
 if(manifest.gatewayConfig.path!=='/etc/blackspire-buyer-writer-gateway/gateway.json'||manifest.serviceDropin.path!=='/etc/systemd/system/blackspire-command.service.d/40-zola-writer.conf')fail();
 const group=execFileSync('/usr/bin/getent',['group','blackspire-writer'],options).trim().split(':');if(group.length!==4||group[0]!=='blackspire-writer'||!/^[1-9][0-9]*$/.test(group[2]))fail();
 ...
 if(!exact(ingress,['version','workspace','bindingFile','writerCredential','issuerCredential'])||ingress.version!==1||ingress.workspace!==manifest.workspace||ingress.bindingFile!=='/etc/blackspire/buyer-writer-binding.json'
 ...
 const dropin=fs.readFileSync(manifest.serviceDropin.path),dropinStat=fs.lstatSync(manifest.serviceDropin.path);
 ...
 const main=Number(execFileSync('/usr/bin/systemctl',['show','--no-pager','--property=MainPID','--value','--','blackspire-command.service'],options).trim());
 const artifactRoot='/opt/blackspire-command/releases/'+releaseSha,api=captureBuyerWriterServiceProcesses({mainPid:main,role:'api',artifactRoot,controlGroup:'/system.slice/blackspire-command.service'});
 ...
 const runtime=await createBuyerWriterRuntimeInspector(context)();context.apiGeneration=runtime.api.invocationId;
 for(const s of snapshots)if(JSON.stringify(s.snapshot)!==JSON.stringify(readRootOwnedJsonDigestSnapshot(s.file,{groupId:s.groupId,maxBytes:s.maxBytes})))fail();
 if(!fs.readFileSync(manifest.serviceDropin.path).equals(dropin))fail();
