...
const P=PARTIAL_RELEASE,fail=()=>{throw new Error('Successor HELD rollover refused; retain exact plan and admission files');};
...
export function createOwnedSuccessorHeldHost(input,journal,{root=RELEASE_ADMISSION_ROOT,recordsRoot='/var/lib/blackspire-operator/owned-successor-holds',files=createOwnedSourceSecurityFiles(),readBytes=readOwnedConfigurationBytes,publishBytes=publishOwnedConfigurationBytes,acquire=acquireReleaseAdmissionLock,stopped=verifyOwnedBuyerMigrationQuiescence,verifySource=verifyReleaseSource,observeLineage=observeOwnedMigrationSuccessor,current=()=>fs.realpathSync('/opt/blackspire-command/current'),groupId=()=>fs.lstatSync(root+'/state.json').gid}={}){
 ...
 return {acquire(){lease=acquire({root,exclusive:true,owner:0,groupId:groupId()});return lease;},authorize,
