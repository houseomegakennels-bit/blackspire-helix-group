...
export async function verifyReleaseArtifactDisk({releaseSha,configuration},{inspect=inspectSealedBuyerWriterArtifact,measure=measureDeploymentHeadroom}={}){
 ...
 if(artifact.releaseSha!==releaseSha||artifact.environment!=='production'||!(/^[a-f0-9]{64}$/).test(artifact.artifactDigest??'')
  ||artifact.status!=='SEALED_ARTIFACT_VERIFIED'||artifact.deployed!==false||artifact.productionAccepted!==false)reject();
