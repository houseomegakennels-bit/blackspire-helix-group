import {successorCanonicalRoot,observeRuntimeSuccessorLineage} from './successor-runtime-predecessor.js';
...
export async function createOwnedSuccessorActivationHost(input,journal){
 ...
 const verifyLineage=()=>observeRuntimeSuccessorLineage({releaseSha:input.releaseSha,operationId:input.operationId,profileDigest:input.profileDigest});
 ...
 const gatewayDeps={verifyRetirement,observeLineage:observeRuntimeSuccessorLineage,releaseEvents:()=>journal.stream('release').events(),verifySuccessorHeld:(_,state)=>{verifyRetirement();const events=journal.stream('release').events(),row=events.findLast(e=>e.type==='release_hold_result'&&e.releaseSha===input.releaseSha);
 ...
 return {
  async fence(){
   verifyReleaseSource(input.releaseSha,{root:successorCanonicalRoot(input.releaseSha),requireRemote:true});const retired=verifyRetirement(),profile=readOwnedDatabaseProfile(),preview=metadata(),before=await runtime();
