...
export function validateOwnedN8nCloudPlan(p){
 ...
 if(p.attempt!==undefined&&(![2,3,4].includes(p.attempt)||!hex(p.predecessorFailureDigest)))fail();
 if(p.attempt===undefined&&p.predecessorFailureDigest!==undefined)fail();
 if([3,4].includes(p.attempt)?!hex(p.predecessorInterruptionDigest):p.predecessorInterruptionDigest!==undefined)fail();
 if(p.attempt===4?!hex(p.predecessorExpiryDigest):p.predecessorExpiryDigest!==undefined)fail();
...
export function buildOwnedN8nCloudWorkflow(plan){
 ...
 return {name:([3,4].includes(p.attempt)?'Zola credential proof attempt'+p.attempt+' ':'Zola credential proof ')+p.challenge,nodes:[
...
export async function prepareOwnedN8nCloudWorkflow(plan,{request,store,fence,now=()=>Date.now()}){
 ...
 if(created){
  ...
  if([2,3,4].includes(plan.attempt)&&!same(validateOwnedN8nCloudWorkflowAcknowledgment(plan,ack),w))fail();
...
export function normalizeOwnedN8nCloudExecutionWorkflow(plan,workflow,raw){
 if(![2,3,4].includes(plan.attempt))return normalizeOwnedN8nCloudWorkflow(plan,raw);
