...
export function validateOwnedN8nCloudPlan(p){
 ...
 if(p.attempt!==undefined&&(![2,3,4,5,6,7].includes(p.attempt)||!hex(p.predecessorFailureDigest)))fail();
 if(p.attempt===undefined&&p.predecessorFailureDigest!==undefined)fail();
 if([3,4,5,6,7].includes(p.attempt)?!hex(p.predecessorInterruptionDigest):p.predecessorInterruptionDigest!==undefined)fail();
 if([4,5,6,7].includes(p.attempt)?!hex(p.predecessorExpiryDigest):p.predecessorExpiryDigest!==undefined)fail();
 if([5,6,7].includes(p.attempt)?!hex(p.predecessorNotReadyDigest):p.predecessorNotReadyDigest!==undefined)fail();
 if([6,7].includes(p.attempt)?!hex(p.predecessorFifthExpiryDigest):p.predecessorFifthExpiryDigest!==undefined)fail();
 if(p.attempt===7?!hex(p.predecessorSixthExpiryDigest):p.predecessorSixthExpiryDigest!==undefined)fail();
...
export function buildOwnedN8nCloudWorkflow(plan){
 ...
 return {name:([3,4,5,6,7].includes(p.attempt)?'Zola credential proof attempt'+p.attempt+' ':'Zola credential proof ')+p.challenge,nodes:[
 {parameters:{},id:'owned-proof-manual',name:'Manual Trigger',type:'n8n-nodes-base.manualTrigger',typeVersion:1,position:[0,0]},
 {parameters:{method:'GET',url:p.origin+p.path,authentication:'genericCredentialType',genericAuthType:'httpHeaderAuth',options:{redirect:{redirect:{followRedirects:false}},timeout:[5,6,7].includes(p.attempt)?70000:40000,response:{response:{responseFormat:'json'}}}},id:'owned-proof-request',name:'Verify stored credential',type:'n8n-nodes-base.httpRequest',typeVersion:4.2,position:[240,0],credentials:{httpHeaderAuth:{id:p.credentialId,name:'ZOLA Buyer writer'}},retryOnFail:false,onError:'stopWorkflow'}
 ],connections:{'Manual Trigger':{main:[[{node:'Verify stored credential',type:'main',index:0}]]}},settings:{executionOrder:'v1',saveDataSuccessExecution:'all',saveDataErrorExecution:'all',saveManualExecutions:true,executionTimeout:[5,6,7].includes(p.attempt)?90:60}};
}
...
export async function prepareOwnedN8nCloudWorkflow(plan,{request,store,fence,now=()=>Date.now()}){
 ...
 if(created){
  ...
  if([2,3,4,5,6,7].includes(plan.attempt)&&!same(validateOwnedN8nCloudWorkflowAcknowledgment(plan,ack),w))fail();
...
export function normalizeOwnedN8nCloudExecutionWorkflow(plan,workflow,raw){
 if(![2,3,4,5,6,7].includes(plan.attempt))return normalizeOwnedN8nCloudWorkflow(plan,raw);
