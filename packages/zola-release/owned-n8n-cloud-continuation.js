import {validateExpired8OwnedN8nRetirement} from './owned-n8n-cloud-expired8.js';
import {validateFailed7OwnedN8nRetirement} from './owned-n8n-cloud-failed7.js';
import {validateOwnedN8nCredentialSave8,SAVED8_UPDATED_AT} from './owned-n8n-credential-save8.js';
import {validateExpired6OwnedN8nRetirement} from './owned-n8n-cloud-expired6.js';
import {validateExpired5OwnedN8nRetirement} from './owned-n8n-cloud-expired5.js';
...
function validateInterruptedContinuation(input,attempt){
 ...
 if([8,9].includes(attempt))validateOwnedN8nCredentialSave8(i.records.plan.credentialMetadata,p.credentialMetadata);else if(!same(p.credentialMetadata,i.records.plan.credentialMetadata))fail();
...
function validateNumberedContinuation({complete,currentOperatorSha},attempt){
 ...
 if(!before||!after||after.updatedAt!==([8,9].includes(attempt)?SAVED8_UPDATED_AT:'2026-09-21T22:55:51.254Z')||!same({...before,updatedAt:null},{...after,updatedAt:null})||!same(after,complete.credentialMetadata)
...
function validateExpiredContinuation(input,attempt){
 ...
 if([8,9].includes(attempt))validateOwnedN8nCredentialSave8(e.records.plan.credentialMetadata,p.credentialMetadata);else if(!same(p.credentialMetadata,e.records.plan.credentialMetadata))fail();
...
export const validateOwnedN8nCloudAttempt5Continuation=input=>validateNotReadyContinuation(input,5);
function validateNotReadyContinuation(input,attempt){
 ...
 if([8,9].includes(attempt))validateOwnedN8nCredentialSave8(n.records.plan.credentialMetadata,p.credentialMetadata);else if(!same(p.credentialMetadata,n.records.plan.credentialMetadata))fail();return {...validateExpiredContinuation(input,attempt),predecessorNotReadyDigest:hash(result),priorAttempt4Outcome:'FAILED_AFTER_PROXY_BEFORE_READY'};
}

export const validateOwnedN8nCloudAttempt6Continuation=input=>validateFifthExpiredContinuation(input,6);
function validateFifthExpiredContinuation(input,attempt){
 const {complete}=input,p=complete?.plan,e=complete?.expired5,result=validateExpired5OwnedN8nRetirement(e?.records,e??{});
 if(p?.predecessorFifthExpiryDigest!==hash(result)||p.challenge===e.records.plan.challenge||p.receiptId===e.records.plan.receiptId||p.path===e.records.plan.path||complete.workflowCreated?.workflow?.id===result.binding.workflowId)fail();
 for(const k of ['releaseSha','operationId','stageAttemptId','credentialId','authorityDigest','originalIntentDigest','reassertionIntentDigest','reassertionAckDigest','reassertionHeadersDigest','reassertionBodyDigest','sourceDigest','profileDigest','ingressDigest','predecessorInterruptionDigest','predecessorFailureDigest','predecessorExpiryDigest','predecessorNotReadyDigest'])if(p[k]!==e.records.plan[k])fail();
 if([8,9].includes(attempt))validateOwnedN8nCredentialSave8(e.records.plan.credentialMetadata,p.credentialMetadata);else if(!same(p.credentialMetadata,e.records.plan.credentialMetadata))fail();
 return {...validateNotReadyContinuation(input,attempt),predecessorFifthExpiryDigest:hash(result),priorAttempt5Outcome:'EXPIRED_WITHOUT_EXECUTION'};
}

export const validateOwnedN8nCloudAttempt7Continuation=input=>validateSixthExpiredContinuation(input,7);
function validateSixthExpiredContinuation(input,attempt){
 const {complete}=input,p=complete?.plan,e=complete?.expired6,result=validateExpired6OwnedN8nRetirement(e?.records,e??{});
 if(p?.predecessorSixthExpiryDigest!==hash(result)||p.challenge===e.records.plan.challenge||p.receiptId===e.records.plan.receiptId||p.path===e.records.plan.path||complete.workflowCreated?.workflow?.id===result.binding.workflowId)fail();
 for(const k of ['releaseSha','operationId','stageAttemptId','credentialId','authorityDigest','originalIntentDigest','reassertionIntentDigest','reassertionAckDigest','reassertionHeadersDigest','reassertionBodyDigest','sourceDigest','profileDigest','ingressDigest','predecessorInterruptionDigest','predecessorFailureDigest','predecessorExpiryDigest','predecessorNotReadyDigest','predecessorFifthExpiryDigest'])if(p[k]!==e.records.plan[k])fail();
 if([8,9].includes(attempt))validateOwnedN8nCredentialSave8(e.records.plan.credentialMetadata,p.credentialMetadata);else if(!same(p.credentialMetadata,e.records.plan.credentialMetadata))fail();
 return {...validateFifthExpiredContinuation(input,attempt),predecessorSixthExpiryDigest:hash(result),priorAttempt6Outcome:'EXPIRED_WITHOUT_EXECUTION'};
}

export const validateOwnedN8nCloudAttempt8Continuation=input=>validateSeventhFailedContinuation(input,8);
function validateSeventhFailedContinuation(input,attempt){
 const {complete}=input,p=complete?.plan,e=complete?.failed7,result=validateFailed7OwnedN8nRetirement(e?.records,e??{});
 const save=validateOwnedN8nCredentialSave8(e.records.plan.credentialMetadata,p?.credentialMetadata);
 if(p?.attempt!==attempt||p.credentialSaveDigest!==hash(save)||!same(complete.credentialSave,save)||p.predecessorSeventhFailureDigest!==hash(result)||p.challenge===e.records.plan.challenge||p.receiptId===e.records.plan.receiptId||p.path===e.records.plan.path||complete.workflowCreated?.workflow?.id===result.binding.workflowId)fail();
 for(const k of ['releaseSha','operationId','stageAttemptId','credentialId','authorityDigest','originalIntentDigest','reassertionIntentDigest','reassertionAckDigest','reassertionHeadersDigest','reassertionBodyDigest','sourceDigest','profileDigest','ingressDigest','predecessorInterruptionDigest','predecessorFailureDigest','predecessorExpiryDigest','predecessorNotReadyDigest','predecessorFifthExpiryDigest','predecessorSixthExpiryDigest'])if(p[k]!==e.records.plan[k])fail();
 return {...validateSixthExpiredContinuation(input,attempt),predecessorSeventhFailureDigest:hash(result),credentialSaveDigest:hash(save),priorAttempt7Outcome:'AUTH_LENGTH_MISMATCH'};
}

export function validateOwnedN8nCloudAttempt9Continuation(input){
 const {complete}=input,p=complete?.plan,e=complete?.expired8,result=validateExpired8OwnedN8nRetirement(e?.records,e??{});
 if(p?.attempt!==9||p.predecessorEighthExpiryDigest!==hash(result)||!same(p.credentialMetadata,e.records.plan.credentialMetadata)||p.challenge===e.records.plan.challenge||p.receiptId===e.records.plan.receiptId||p.path===e.records.plan.path||complete.workflowCreated?.workflow?.id===result.binding.workflowId)fail();
 for(const k of ['releaseSha','operationId','stageAttemptId','credentialId','authorityDigest','originalIntentDigest','reassertionIntentDigest','reassertionAckDigest','reassertionHeadersDigest','reassertionBodyDigest','sourceDigest','profileDigest','ingressDigest','predecessorInterruptionDigest','predecessorFailureDigest','predecessorExpiryDigest','predecessorNotReadyDigest','predecessorFifthExpiryDigest','predecessorSixthExpiryDigest','predecessorSeventhFailureDigest','credentialSaveDigest'])if(p[k]!==e.records.plan[k])fail();
 return {...validateSeventhFailedContinuation(input,9),predecessorEighthExpiryDigest:hash(result),priorAttempt8Outcome:'EXPIRED_AND_CLEANED_BEFORE_TWO_LATE_EXECUTIONS'};
}
