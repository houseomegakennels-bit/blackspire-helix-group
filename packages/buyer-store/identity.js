...
export function resolveBuyerStoreApiGroup({run=execFileSync,uid=process.getuid(),groups=process.getgroups()}={}){
 const options={encoding:'utf8',timeout:1000,maxBuffer:4096,env:{PATH:'/usr/bin:/bin'}};
 const user=run('/usr/bin/getent',['passwd','blackspire-api'],options).trim().split(':');
 const group=run('/usr/bin/getent',['group','blackspire-api'],options).trim().split(':');
 const gid=Number(group[2]);
 if(user.length!==7||user[0]!=='blackspire-api'||Number(user[2])!==uid||uid===0||group.length!==4||group[0]!=='blackspire-api'||!Number.isInteger(gid)||gid<=0||!groups.includes(gid))fail();
 return gid;
}
