...
export function createBuyerStoreLocalServer({configuration,userHandler,readCapabilityProfiles,validateInput,now=Date.now}={}){
 ...
 const server=net.createServer({allowHalfOpen:true},socket=>{
  ...
  socket.once('end',async()=>{
   ...
   try{
    ...
    const result=request.lane==='user'
...
export async function listenBuyerStore(server,{uid=process.getuid(),apiGroupId,groups=process.getgroups(),io=fs}={}){
 if(!Number.isInteger(uid)||uid===0||!Number.isInteger(apiGroupId)||apiGroupId===process.getgid()||!groups.includes(apiGroupId))fail();
 ...
 io.chownSync(path.dirname(BUYER_STORE_SOCKET),uid,apiGroupId);
 await new Promise((resolve,reject)=>{server.once('error',reject);server.listen(BUYER_STORE_SOCKET,()=>{io.chownSync(BUYER_STORE_SOCKET,uid,apiGroupId);io.chmodSync(BUYER_STORE_SOCKET,0o660);resolve();});});
 return server;
}
