...
-- Restrictive owner fences remain effective even if an imported permissive
-- PUBLIC policy exists; owned-cluster migration also audits baseline grants.
CREATE POLICY buyer_repository_jobs_owner_fence ON public."SearchJob" AS RESTRICTIVE TO buyer_repository_user USING(user_id=nullif(current_setting('request.jwt.claim.sub',true),'')::uuid) WITH CHECK(user_id=nullif(current_setting('request.jwt.claim.sub',true),'')::uuid);
CREATE POLICY buyer_repository_reports_owner_fence ON public."BuyerReport" AS RESTRICTIVE FOR SELECT TO buyer_repository_user USING(EXISTS(SELECT 1 FROM public."SearchJob" j WHERE j.id=search_job_id AND j.user_id=nullif(current_setting('request.jwt.claim.sub',true),'')::uuid));
CREATE POLICY buyer_repository_exports_owner_fence ON public.exports AS RESTRICTIVE TO buyer_repository_user USING(user_id=nullif(current_setting('request.jwt.claim.sub',true),'')::uuid) WITH CHECK(user_id=nullif(current_setting('request.jwt.claim.sub',true),'')::uuid AND (search_job_id IS NULL OR EXISTS(SELECT 1 FROM public."SearchJob" j WHERE j.id=search_job_id AND j.user_id=nullif(current_setting('request.jwt.claim.sub',true),'')::uuid)));
