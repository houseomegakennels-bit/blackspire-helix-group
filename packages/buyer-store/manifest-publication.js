import {acquireReleaseAdmissionLock,RELEASE_ADMISSION_LOCK} from '../shared/release-admission.js';
...
export async function publishBuyerStoreInstalledManifest(binding,{restore=false,paths=defaultPaths,io=fs,uid=process.getuid(),readConfig=readConfiguration,observeGenerations=observe,inspect=inspectSealedBuyerWriterArtifact,readProtected=readRootOwnedJson}={}){
 ...
 const bytes=v=>Buffer.from(JSON.stringify(v)+'\n');const protectedFiles=createBuyerStoreProtectedFiles({io});
 ...
 const directory=p=>protectedFiles.directory(p,{create:true});
 const retain=(p,value)=>protectedFiles.record(p,value);
 ...
 let lock;try{
  if(exists(paths.root+'/lock'))fail();
  protectedFiles.publish(paths.root+'/admission.lock',Buffer.from(RELEASE_ADMISSION_LOCK),{gid:0,mode:0o640});
  lock=acquireReleaseAdmissionLock({root:paths.root,exclusive:true,owner:0,groupId:0,io});
  const current=()=>{
   if(!exists(paths.manifest))return null;
   const value=JSON.parse(protectedFiles.read(paths.manifest,{gid,mode:0o640})),stat=io.lstatSync(paths.manifest);
  ...
  if(JSON.stringify(actual)!==JSON.stringify(destination)){
   if(destination===null)io.unlinkSync(paths.manifest);
    ...
    if(exists(temporary)){
     const retained=JSON.parse(protectedFiles.read(temporary,{gid,mode:0o640}));
