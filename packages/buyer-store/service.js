...
export function validateBuyerStoreInput(operation,input){
 ...
 case 'export-create':valid=shape(input,['id','searchJobId','fileName','storagePath','rowCount'])&&uuid(input.id)&&(input.searchJobId===null||uuid(input.searchJobId))&&string(input.fileName,256)&&string(input.storagePath,1024)&&!input.storagePath.split('/').includes('..')&&integer(input.rowCount,0,1000000);break;
