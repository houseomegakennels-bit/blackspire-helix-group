...
## Hermes Intelligence Layer (Milestone 2 — merged, development-only)

- **A development-only real-provider runtime is merged through PR #56.** Real execution is **disabled by default**, requires the development profile, explicit feature flag, provider allowlist, registered/allowlisted workspace root, positive bounded spend reservation, and valid scoped approval; it is **refused under the production profile even if a credential is present**. The first real adapter is the **non-agentic Anthropic Messages API (Claude)** — pure text I/O, no shell/agentic surface.
