...
import pg from 'pg';
import {authenticateBuyerWriterProductionIdentity,provisionOwnedBuyerWriterProduction} from '../packages/buyer-writer/production-provisioner.js';
...
try{
  ...
  if(args[2]!=='/etc/blackspire/owned-postgres/management.json')stopped();
  const result=await provisionOwnedBuyerWriterProduction({mode:args[0].slice(2),managementConfigPath:args[2],lookupWriterGroup,connect,authenticate});
