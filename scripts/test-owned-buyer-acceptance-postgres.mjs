...
try{
 ...
 for(const name of names){
  const id=requireSuccess(run(['create','--name',name,'--label',`blackspire.test-owner=${owner}`,'--network',network,
   ...
   '-e',`POSTGRES_USER=${name.endsWith('-target')?'blackspire_cluster_admin':'postgres'}`,'-e','POSTGRES_HOST_AUTH_METHOD=trust','-e','POSTGRES_DB=postgres',image]));ids.push(id);assert.match(id,/^[a-f0-9]{64}$/);requireSuccess(run(['start',id]));
  ...
  let ready=false;for(let n=0;n<40;n++){if(run(['exec',id,'pg_isready','-h','127.0.0.1','-U',name.endsWith('-target')?'blackspire_cluster_admin':'postgres','-d','postgres']).status===0){ready=true;break;}await new Promise(resolve=>setTimeout(resolve,250));}assert.ok(ready,'disposable PostgreSQL readiness timed out');
 }
 const proof=spawnSync(process.execPath,['--max-old-space-size=256',new URL('./test-owned-buyer-acceptance-session.mjs',import.meta.url).pathname],{
