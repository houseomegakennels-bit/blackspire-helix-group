...
if [[ -e "$env_file" || -L "$env_file" ]]; then
  if [[ -L "$env_file" ]]; then
    ...
    missing_keys=""
    for key in NODE_ENV BLACKSPIRE_RUNTIME_MODE BLACKSPIRE_STATE_OWNER \
...
if [[ -e "$api_env_file" || -L "$api_env_file" ]]; then
  if [[ -L "$api_env_file" || ! -f "$api_env_file" ]]; then
    ...
    missing_api_keys=""
    for key in COMMAND_ADMIN_PASSWORD_HASH SESSION_SECRET ALLOW_BEARER_AUTH COMMAND_ADMIN_TOKEN BLACKSPIRE_AUTHORITY_CONSUMER_TOKEN; do
...
PREPARATION (safe where described, no activation)
  ...
  1. Create the shared and API-only environment files. Each file is created with mode 0640 in
     one step, so it is never briefly world-readable:
       ...
       # and edit $api_env_file to set COMMAND_ADMIN_PASSWORD_HASH, SESSION_SECRET, and
       # BLACKSPIRE_AUTHORITY_CONSUMER_TOKEN. The authority token must be the same protected
       # value configured for the Vercel receiver. Never put these values in the shared $env_file.
