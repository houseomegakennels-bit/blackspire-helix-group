...
const report=runDisposableRestoreCutover({repository,root:value('--root'),environment:value('--environment'),operatorAck:value('--operator-ack'),expectedCommit:value('--commit'),rollbackCommit:value('--rollback'),fault:value('--fault'),treeClean:status.status===0&&status.stdout==='',cleanup:!args.includes('--preserve-fixture')});
