...
import {
  inspectOwnedBuyerWriterGatewayConfigurationUpgrade,
  upgradeOwnedBuyerWriterGatewayConfiguration,
} from '../packages/buyer-writer/gateway-configuration-upgrade.js';
import {
  BUYER_WRITER_GATEWAY_CONFIG_FILE,
...
import {readOwnedDatabaseProfile} from '../packages/buyer-writer/database-profile.js';
import {assertRetiredGatewaySnapshot} from '../packages/buyer-writer/owned-gateway-transition.js';
const BUYER_WRITER_GATEWAY_UPGRADE_STATE='/var/lib/blackspire-operator/owned-gateway-transition';
let ownedSource,ownedProfile;
...
try{
  ...
  ownedSource=readRootOwnedJsonSnapshot(sourceFile,{groupId:0,maxBytes:65536});ownedProfile=readOwnedDatabaseProfile();
  ...
  const {verifyReleaseSource}=await import('/mnt/blackspire-builds/development-cache/0/workspaces/zola-final-release-20260921/packages/zola-release/commander-host.js');verifyReleaseSource(releaseSha);
  ...
  if(mode==='--rollback'){
    ...
    const result=await rollbackBuyerWriterGatewayConfigurationFile({
      ...bound,writerGroupId:groupId,stateDirectory:BUYER_WRITER_GATEWAY_UPGRADE_STATE,proveQuiesced});
    ...
    const retiredState=readRootOwnedJsonSnapshot('/var/lib/blackspire-operator/gateway-configuration-upgrade/63017917-49de-4942-bc8f-2aab9463fb78.state.json',{groupId:0,maxBytes:4096}).value;
    assertRetiredGatewaySnapshot({current:oldConfiguration,state:retiredState});
    if(config.writerCredential!==ownedSource.value.writerCredential||config.issuerCredential!==ownedSource.value.issuerCredential)fail();
    const inspection=inspectOwnedBuyerWriterGatewayConfigurationUpgrade({
      ...bound,oldConfiguration,newConfiguration:gatewayConfig,ownedProfile,ownedSource:ownedSource.value});
    if(mode==='--inspect')process.stdout.write(JSON.stringify(inspection)+'\n');
      ...
      const result=await reconcileBuyerWriterGatewayConfigurationFile({
        ...
        writerGroupId:groupId,stateDirectory:BUYER_WRITER_GATEWAY_UPGRADE_STATE,proveQuiesced});
      process.stdout.write(JSON.stringify(result)+'\n');
    }else{
      safeStateDirectory();
      ...
      const controls=createBuyerWriterGatewayConfigurationFileControls({
        ...bound,writerGroupId:groupId,stateDirectory:BUYER_WRITER_GATEWAY_UPGRADE_STATE,proveQuiesced});
      const result=await upgradeOwnedBuyerWriterGatewayConfiguration({
        ...bound,oldConfiguration,newConfiguration:gatewayConfig,controls,ownedProfile,ownedSource:ownedSource.value,
