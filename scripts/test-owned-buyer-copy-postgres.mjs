...
try{
 ...
 for(const name of names){
  ...
  let ready=false;for(let n=0;n<40;n++){if(run(['exec',id,'pg_isready','-U','postgres','-d','postgres']).status===0){ready=true;break;}await new Promise(resolve=>setTimeout(resolve,250));}assert.ok(ready,'disposable PostgreSQL readiness timed out');
 }
