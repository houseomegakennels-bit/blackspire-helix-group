...
import {prepareOwnedZolaConfigurationInstall,installZolaConfiguration} from '../packages/zola-release/configuration-install.js';
...
if(process.argv[1]&&import.meta.url===pathToFileURL(process.argv[1]).href){
  ...
  try{
    ...
    const plan=await prepareOwnedZolaConfigurationInstall({releaseSha,configurationFile});
    let result=plan;
    if(mode==='--install'){
