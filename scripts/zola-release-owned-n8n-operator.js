import {validateOwnedN8nCloudHistory,createOwnedN8nCloudContinuation,assertOwnedN8nCloudContinuationOperator,validateOwnedN8nCloudAttempt2Continuation,validateOwnedN8nCloudAttempt3Continuation,validateOwnedN8nCloudAttempt4Continuation} from '../packages/zola-release/owned-n8n-cloud-continuation.js';
...
try{
 ...
 if(process.getuid?.()!==0||process.versions.node!=='22.23.1'||!['--release','--reassert-credential','--release-cloud-proof','--release-cloud-proof-attempt2','--release-cloud-proof-attempt3','--release-cloud-proof-attempt4'].includes(mode)||process.argv.length!==4)fail();
 ...
 const cloudProof=async b=>{
  const readers=await import(mode==='--release-cloud-proof-attempt4'?'../packages/zola-release/owned-n8n-cloud-attempt4-host.js':mode==='--release-cloud-proof-attempt3'?'../packages/zola-release/owned-n8n-cloud-attempt3-host.js':'../packages/zola-release/owned-n8n-cloud-host.js');
  const readComplete=mode==='--release-cloud-proof-attempt4'?readers.readCompletedOwnedN8nCloudProofAttempt4:mode==='--release-cloud-proof-attempt3'?readers.readCompletedOwnedN8nCloudProofAttempt3:mode==='--release-cloud-proof-attempt2'?readers.readCompletedOwnedN8nCloudProofAttempt2:readers.readCompletedOwnedN8nCloudProof;
  ...
  if(mode==='--release-cloud-proof-attempt4')validateOwnedN8nCloudAttempt4Continuation({complete,currentOperatorSha:operatorSha});
 ...
 const {assertConfigured,synchronize}=['--release-cloud-proof','--release-cloud-proof-attempt2','--release-cloud-proof-attempt3','--release-cloud-proof-attempt4'].includes(mode)
