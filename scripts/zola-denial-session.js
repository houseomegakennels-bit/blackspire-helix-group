...
import {readReleaseProtectedBytes} from '../packages/zola-release/commander-host.js';
...
try{
 ...
 const keys=mode==='--issue'?'deniedPrincipal,outputPath,releaseSha,runId,workspace':'receiptFile';
 ...
 if(mode==='--issue'){
  runtime=await openDenialSessionRuntime(input.releaseSha);profile=runtime.profile;pid=profile.context.apiPid;
  if(input.workspace!==profile.context.workspace)fail();
  ...
  if(environment.get('BLACKSPIRE_DB_PATH')!==DATABASE||environment.get('NODE_ENV')!=='production'||!(/^[A-Za-z0-9._:-]{1,128}$/).test(operator??'')||stat.uid!==profile.context.apiUid)fail();
 ...
 if(mode==='--issue')await runtime.assertCurrent();assertDatabase();
 let result;
 if(mode==='--issue'){
  result=service.issue({operatorPrincipal:operator,deniedPrincipal:input.deniedPrincipal,workspace:input.workspace,runId:input.runId,releaseSha:input.releaseSha},value=>{
   runtime.assertHeld();assertDatabase();const protectedReceipt={...value,databaseIdentity};writeZolaActivationProfile(input.outputPath,protectedReceipt);
   if(digest(JSON.parse(readReleaseProtectedBytes(input.outputPath,16384)))!==digest(protectedReceipt))fail();
  });
  await runtime.assertCurrent();assertDatabase();
...
}finally{try{runtime?.close();}catch{process.exitCode=1;}try{service?.close();}catch{process.stderr.write('Delegated denial session database close failed; retain receipt.\n');process.exitCode=1;}}
