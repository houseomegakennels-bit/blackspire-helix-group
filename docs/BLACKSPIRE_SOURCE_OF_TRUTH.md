...
Gate 3 data-protection rehearsal remains BLOCKED as of 2026-07-22 UTC. PR #28 explicit-migration repair was independently reviewed and merged by `691973870e0048f273fa7e9251d7f78776e3612b`; migration safety is green. A restricted-staging rebuild then stopped before activation when its new immutable release `releases/691973870e0048f273fa7e9251d7f78776e3612b` was `0700 root:root`, preventing the `blackspire` runtime from traversing it. The prior staging release `releases/0a9affacaf13dd1b040c5d96eb112d979ab59444` was restored and remains active on loopback `127.0.0.1:8788`; public HTTPS health is 200. A source-only repair branch must restore the reviewed `root:blackspire` immutable-release contract (directories/executables `0755`, ordinary files `0644`) and receive independent review before another staging rebuild. `current`, `/etc/blackspire/command.env`, and the production `command.sqlite` remain absent; `blackspire-command.service` is disabled/inactive. This is not production activation or a production cutover. No Gate 3 backup, restore, WAL rehearsal, timer installation, production-path write, or service restart occurred. `READY_TO_REQUEST_GATE_4_PRODUCTION_ACTIVATION: no` and `READY_FOR_VPS_PRODUCTION_DEPLOYMENT: no`.
