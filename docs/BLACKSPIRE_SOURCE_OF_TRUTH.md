...
- Pre-staging follow-up test-contract validation discovered 29 intended root `tests/**/*.test.js` files. The prior shell glob happened to execute the 28 pre-existing top-level files in Bash, but it did not establish a recursive contract or record actual test-worker execution. The follow-up runner now supplies one sorted explicit file list to a single concurrency-1 Node TAP run, strips ordinary migration permission, and verifies every expected file worker executed exactly once; the first expanded run passed 261 tests with no failures, skips, or cancellations under Node 22.23.1.
