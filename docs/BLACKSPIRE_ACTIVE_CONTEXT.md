...
A subscription-authenticated Codex worker adapter is implemented and locally tested. Codex CLI 0.144.5 reported ChatGPT authentication. The single approved live invocation failed without a contract-valid response; it used no API key, retry, fallback, or observed tool call, canonical state remained failed, and disposable state was removed. Another live invocation is not authorized.
