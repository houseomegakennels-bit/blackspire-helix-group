...
`scripts/zola-denial-session.js --issue <protected-input.json>` is an explicit root-authenticated delegation to an **existing** distinct active canonical admin principal. It requires no active grants in any workspace, exact clean release source or execution from the fixed sealed `/opt/blackspire-command/releases/<releaseSha>` artifact, actual installed-v4 canonical API/worker identity and the canonical API's configured operator/database. Issue input has exactly `deniedPrincipal`, `outputPath`, `releaseSha`, `runId`, and `workspace`; `outputPath` is a new exclusive protected receipt file. Invoke the exact candidate artifact CLI before candidate collection and the exact new-main artifact CLI after cutover. The installed manifest, deployed artifact and current generations are re-observed while a shared HELD lease spans issuance; `configurationFile` is no longer accepted. It creates a session through the shared session subsystem, with an audited maximum 15-minute lifetime. This is operator-issued authentication, not a second person's password login. It creates no principal or grant. The receipt is fsynced before transaction commit; on uncertainty, preserve it and reconcile instead of repeating issuance.
...
Before the six normal admissions, every collector mode now requires an authenticated negative admission from the existing distinct principal. The HTTP boundary obtains that session's CSRF token privately and requires exact authorization `404 {"error":"not found"}`; authentication, CSRF, rate-limit and emergency-stop failures do not count. No active grant may exist for that principal. A durable `denial_intent` precedes the POST; confirmation requires unchanged bounded read-only snapshots of `tasks`, `unified_inputs`, `provider_attempts` and `provider_usage`, plus unchanged runtime generation.

An unconfirmed negative intent is never retried, even if no task exists. Confirmed evidence can be reused only after current identity, no-grant authority and generation checks; the normal six tasks may legitimately change table contents afterward. Old journals containing admissions but no negative-denial evidence refuse. The report labels reuse and scope explicitly. This proves Command admission refusal and zero observed persisted change across that interval, not all application-owner policies, transient mutation attempts, audit/session immutability or process-wide provider/egress containment.
