...
The optional `migrationConfigurationFile` field must name the exact bundle's `migration-input.json`, beside `migration-manifest.json`, `application-body.sql` and `application.sql`. The preflight regenerates all payloads, retains sanitized hashes in version-two release history and repeats verification before its final identity observation. Older version-one histories remain inspectable.

The internal `executeReleaseNativeMigration` adapter records global intent before the existing standalone migration claim and then invokes the guarded native executor on the enclosing commander's dedicated TLS session. It is not exposed by the CLI and does not establish intake, acceptance or provider authority. Any existing migration intent blocks a fresh apply and preflight, including after failed claims or uncertain acknowledgement. Reconciliation verifies exact history in a read-only transaction; absence never permits retry. The enclosing activation and release adapters remain unwired.

Artifact/disk preflight consumes only `SEALED_ARTIFACT_VERIFIED` evidence with `deployed:false` and `productionAccepted:false`. Sealed verification requires no deployment record, verifies the same protected tree and digest, and permits only the missing deployment-record reason. The separate runtime inspector still requires a real deployment record. Neither path writes one.
