...
- Round 6 supersedes the paragraph's final ten-table-set claim: a raw read of the populated `hermes_memory_candidates` relation survived at 21/21 because that table was not manually named. The behavioural guard now discovers and instruments every user table from the live disposable schema, excluding only `hermes_outcome_evaluations` and the three authorization tables for the documented reasons. The reproduced mutant now fails with the new relation recorded; representative authorized reads retain non-vacuity witnesses. Node 22.23.1 validation of the corrected focused guard passes. Fresh independent review, merge, deployment, and production effect are not yet claimed.
