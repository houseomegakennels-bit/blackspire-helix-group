...
- The read-ordering regression is pinned three ways, and the load-bearing guard is behavioural rather than textual. **The earlier claim on this line — that the source-text pins were what covered work confined to the components or workflow-evidence tables — was false and is retracted.** A source-text assertion, bounded to the function body and stripped of comments, requires the authorization decision to precede every integrity entry point the read path could call; it is retained only as a fast, readable first failure and is **not** coverage, because it matches helper names literally and four mutants were reproduced that evade it while doing identical pre-authorization work — a string literal containing `//` (which the comment stripper then erases to end of line), a one-line alias function, an alias held in a lookup object, and a bare `loadEvidence` call behind the same string trick. Each restored the oracle to ~150x against a ~64x head baseline, versus ~130x before the ordering fix. The first behavioural guard hides `hermes_outcome_corrections` and `hermes_outcome_source_events` so a pre-authorization read of either throws, which cannot reach work `provenanceMatches` swallows in its own try/catch. The second behavioural guard closes exactly that gap: it swaps `hermes_workflow_runs` and `hermes_outcome_evaluation_components` for views over the real tables whose `WHERE` clause calls a UDF recording the access, so a pre-authorization touch is *observed* rather than needing to throw, independent of source shape. It asserts the unauthorized read touches neither sentinel, asserts non-vacuously that the authorized read touches both, and asserts the schema is restored exactly. Thirteen mutants were executed against the set — the previous six plus the four evasions above, a trailing-comment variant, a dead-`if` variant, and a URL-string variant — and all thirteen fail the test (21 tests, 20 pass, 1 fail each); the unmutated head passes 21/21.
