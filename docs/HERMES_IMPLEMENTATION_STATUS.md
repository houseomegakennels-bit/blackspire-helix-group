...
- The read-ordering regression is pinned two ways. A source-text assertion, bounded to the function body and stripped of comments, requires the authorization decision to precede every integrity entry point the read path could call, not merely the aggregate `readableEvaluation`. A behavioural guard hides `hermes_outcome_corrections` and `hermes_outcome_source_events` so a pre-authorization read of either throws; it does **not** cover work confined to the components or workflow-evidence tables, because `provenanceMatches` swallows its own exceptions, and that limit is stated in the test. Six mutants were executed against the pair - full revert, combined reintroduction, digest-only, source-events-only, components-only, and a try/catch-wrapped revert - and all six fail the test.
