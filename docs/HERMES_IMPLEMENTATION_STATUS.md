...
## Milestone 3C — Approval-gated memory-candidate review (draft PR, in progress)
...
- **Next:** re-review/successor chains, a workspace-scoped review queue, then promotion — each a separate dependency-ordered milestone. Promotion must not reuse `evaluation.correct` as its authority without an explicit decision.
